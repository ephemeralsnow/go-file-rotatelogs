...
func TestLogRotateAbsPath(t *testing.T) {
...
func TestLogRotateRelPath(t *testing.T) {
	dir, err := ioutil.TempDir("", "file-rotatelogs-test")
	if err != nil {
		t.Errorf("Failed to create temporary directory: %s", err)
	}
	...
	err = os.Chdir(dir)
	if err != nil {
		t.Errorf("Failed to change current directory: %s", err)
	}

	err = os.Mkdir("rel", 0700)
	if err != nil {
	...
	rl := NewRotateLogs(filepath.Join("rel", "log%Y%m%d%H%M%S"))
	defer rl.Close()

	rl.MaxAge = 86400 * time.Second
	rl.LinkName = filepath.Join("rel", "log")
	...
	linkTarget, err := filepath.Rel(linkDir, newfn)
	if err != nil {
		t.Errorf("Failed to rel %s: %s", linkDir, err)
	}

	if linkDest != linkTarget {
		t.Errorf(`Symlink destination does not match expected filename ("%s" != "%s")`, linkTarget, linkDest)
	}
}
